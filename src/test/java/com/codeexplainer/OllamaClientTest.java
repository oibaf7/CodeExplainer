...
class OllamaClientTest {
    ...
    @Test
    void httpClientMockTestValidResponse() throws ExecutionException, InterruptedException {
        String fakeBody = "{\"response\":\"<p>This is an explanation</p>\",\"done\":true}";

        HttpResponse<String> mockResponse = mock(HttpResponse.class);
        when(mockResponse.body()).thenReturn(fakeBody);

        when(httpClient.sendAsync(any(HttpRequest.class), any(HttpResponse.BodyHandler.class)))
                .thenReturn(CompletableFuture.completedFuture(mockResponse));

        String result = ollamaClient.explain("int x = 1;").get();

        assertEquals("<p>This is an explanation</p>", result);
    }

    @Test
    void httpClientMockTestInvalidResponse() throws ExecutionException, InterruptedException {
        String fakeBody = "{error}";
        ...
        assertEquals("Wrong response format! Verify the AI model is running at the right port!", result);
    }
}